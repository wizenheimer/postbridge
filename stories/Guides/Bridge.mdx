...
## ️ Important: Multi-Writer Considerations
...
```typescript
const conn = await bridge.connect(schema);
console.log(conn.id); // → "abc-123-def-456"
...
// → "Connected tabs: tab-abc, tab-def, tab-xyz"
...
console.log(`Session ID: ${conn.id}`); // → consistent across reloads
...
## 🆚 Bridge vs PostBridge RPC

| Feature       | PostBridge RPC                 | Bridge          |
| ------------- | --------------------------- | --------------------- |
| Communication | 1:1 (host ↔ guest)         | 1:N (tab ↔ all tabs) |
